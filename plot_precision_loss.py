...
Rounding introduces error in the range [-0.5, 0.5] for each of the N coefficients.
...
The theoretical precision error bound is: sqrt(N) * 0.5 / scale
For a scaling factor of 2^k and modulus degree N, the bound is sqrt(N) * 2^(-k-1).
"""
...
def measure_precision_loss(message: np.ndarray, scale: float, N: int) -> float:
    """
    Encode and decode a message with given scale and N, return the max absolute error.
    """
    params = EncodingParams(scale=scale, poly_modulus_degree=N)
    encoded = encode(message, params)
    decoded = decode(encoded, params)
    error = np.max(np.abs(message - decoded[:len(message)]))
    return error
...
def main():
    ...
    for k in k_values:
        ...
        theoretical = np.sqrt(N) * 0.5 / scale
    ...
    # Calculate mean diffs for the second plot
    mean_precision_losses = [np.mean(losses) for losses in all_precision_losses]
    diffs = np.abs(np.array(mean_precision_losses) - np.array(theoretical_losses))

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 12))

    # First plot: Precision loss comparison with distribution
    # Plot all individual measurements as scatter points
    for i, (scale, losses) in enumerate(zip(scaling_factors, all_precision_losses)):
        ax1.scatter([scale] * len(losses), losses, alpha=0.3, s=10, color='blue')

    # Plot mean values
    ax1.loglog(scaling_factors, mean_precision_losses, 'o-', label='Mean Measured Loss',
               linewidth=2, markersize=6, color='darkblue')

    # Plot theoretical bound
    ax1.loglog(scaling_factors, theoretical_losses, '--', label=f'Theoretical Bound (√{N} × 0.5 / scale)',
               linewidth=2, alpha=0.7, color='red')

    ax1.set_xlabel('Scaling Factor (2^k)', fontsize=12)
    ax1.set_ylabel('Precision Loss (Max Absolute Error)', fontsize=12)
    ax1.set_title(f'CKKS Encoding Precision Loss vs Scaling Factor (N={N}, {num_trials} trials per scale)', fontsize=14)
    ax1.grid(True, which="both", ls="-", alpha=0.2)
    ax1.legend(fontsize=11)

    # Second plot: Relative error (normalized by theoretical)
    # Calculate relative errors for each trial
    for i, (scale, losses, theoretical) in enumerate(zip(scaling_factors, all_precision_losses, theoretical_losses)):
        relative_errors = [(loss - theoretical) / theoretical for loss in losses]
        ax2.scatter([scale] * len(relative_errors), relative_errors, alpha=0.3, s=10, color='purple')

    # Plot mean relative error
    mean_relative_errors = [(mean - theo) / theo for mean, theo in zip(mean_precision_losses, theoretical_losses)]
    ax2.semilogx(scaling_factors, mean_relative_errors, 'o-', color='darkviolet',
                 label='Mean Relative Error', linewidth=2, markersize=6)

    # Add a horizontal line at 0
    ax2.axhline(y=0, color='black', linestyle='--', linewidth=1, alpha=0.5, label='Perfect Match')

    ax2.set_xlabel('Scaling Factor (2^k)', fontsize=12)
    ax2.set_ylabel('Relative Error: (Measured - Theoretical) / Theoretical', fontsize=12)
